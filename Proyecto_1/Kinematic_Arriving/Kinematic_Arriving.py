...
# Implementación del algoritmo Kinematic Arriving
import pygame, sys
...
max_speed = 5
radius = 10
kinematic_arrive = KinematicArrive(cat, mouse, max_speed, radius)
...
def recargaPantalla():
    ...
    steering = kinematic_arrive.getSteering()
    if steering:
        # Se actualiza la posición del gato
        cat.position.x += steering.velocity.x
        cat.position.z += steering.velocity.z
        pos_inicial_x = cat.position.x
        pos_inicial_y = cat.position.z
    
    # Se dibuja al gato en su nueva posición
    if steering and steering.velocity.x < 0:
        PANTALLA.blit(saltaIzquierda[cuentaPasos // 1], (int(pos_inicial_x), int(pos_inicial_y)))
    elif steering and steering.velocity.x > 0:
        PANTALLA.blit(saltaDerecha[cuentaPasos // 1], (int(pos_inicial_x), int(pos_inicial_y)))
    else:
        PANTALLA.blit(quieto, (pos_inicial_x, pos_inicial_y))
